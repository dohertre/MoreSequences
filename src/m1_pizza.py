...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    radius = 10
    small = generate_points_on_circle(circle, number_of_points)
    for k in range(number_of_points):
        circles = rg.Circle(small[k], radius)
...
def run_test_polygon():
    ...
    # ------------------------------------------------------------------
    title = 'POLYGON test 3:  3 segments with thick blue lines;  6 with medium red lines.'
    window = rg.RoseWindow(550, 400, title)
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    # ------------------------------------------------------------------
    circle.attach_to(window)
    circles = generate_points_on_circle(circle, number_of_segments)
    for k in range(number_of_segments):
        lines = rg.Line(circles[k-1], circles[k])
        lines.outline_color = color
        lines.thickness = thickness
        lines.attach_to(window)
...
def run_test_fancy_polygon():
    ...
    # ------------------------------------------------------------------
    title = 'FANCY POLYGON test 4:  6 blue lines on blue circle, hops = 2.'
    window = rg.RoseWindow(480, 350, title)

    circle = rg.Circle(rg.Point(240, 165), 150)
    circle.fill_color = 'maroon'
    fancy_polygon(window, circle, 6, 2, 'blue', 2)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    # ------------------------------------------------------------------
    circle.attach_to(window)
    circles = generate_points_on_circle(circle, number_of_lines)
    for k in range(number_of_lines):
        lines = rg.Line(circles[k - hops_to_next_point], circles[k])
        lines.outline_color = color
        lines.thickness = thickness
        lines.arrow = 'last'
        lines.attach_to(window)
